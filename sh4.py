...
import random  # Added for unique elements
...
# Unique elements to add to prompts
UNIQUE_MARKERS = [
    "🦄UNICORN-{}-MARKER🦄",
    "🚀ROCKET-{}-SENTINEL🚀",
    "🌈RAINBOW-{}-BEACON🌈",
...
def add_unique_marker(prompt: str) -> str:
    """Add a unique marker to the prompt to make it identifiable."""
    marker_template = random.choice(UNIQUE_MARKERS)
    unique_id = str(uuid.uuid4())[:8]
    marker = marker_template.format(unique_id)
    enhanced_prompt = f"{prompt} [UNIQUE-ID: {marker}]"
    logger.info(f"🎯 Added unique marker: {marker}")
...
class StagehandTool:
    ...
    async def inject_prompt(self, prompt_text: str) -> dict[str, Any]:
        """Inject prompt directly into the input field with unique marker."""
        # Add unique marker to the prompt
        enhanced_prompt = add_unique_marker(prompt_text)

        logger.info(f"💉 Original prompt: {prompt_text[:50]}...")
        logger.info(f"🦄 Enhanced prompt: {enhanced_prompt[:80]}...")

        return self._request("POST", "/inject-prompt", {"prompt": enhanced_prompt})
...
class PydanticAIWebAutomationAgent:
    ...
    def __init__(self) -> None:
        ...
        async def inject_prompt(ctx: RunContext[AgentDependencies], prompt_text: str = "") -> str:
            """Inject prompt directly into ChatGPT input field with unique marker - USE THIS INSTEAD OF TYPING!"""
            ...
            console.print(f"[cyan]💉 Injecting prompt with unique marker[/cyan]")

            result = await ctx.deps.stagehand_tool.inject_prompt(prompt_text)

            if result["success"]:
                ctx.deps.execution_history.append(f"Injected prompt with marker: {prompt_text[:50]}...")
                logger.success("✅ Prompt injected successfully")
                console.print(f"[green]✅ Prompt injected with unique marker[/green]")
                return f"✅ Prompt injected with unique marker. Now click the send button (arrow →)!"
        ...
        async def get_current_state(ctx: RunContext[AgentDependencies]) -> str:
            ...
            state_table.add_row("Raw Data Records", str(len(state.raw_extraction_data)))
...
- Raw Data Records: {len(state.raw_extraction_data)}
...
3. FOR CHATGPT/AI SERVICES - NEW SIMPLIFIED STEPS:
   ...
   - inject_prompt("your prompt text here") ← USE THIS! ADDS UNIQUE MARKER!
   - observe_page("Verify prompt was injected with unique marker")
   ...
   - extract_ai_response() ← USE THIS! LOGS RAW DATA!
...
5. EXTRACTION RULES:
   ...
   - All extractions now log raw data automatically
...
6. STATE AWARENESS:
   ...
   - Raw data is automatically included in saves
...
- inject_prompt: Inject prompt with UNIQUE MARKER (NO TYPING!)
...
- extract_dom_content: Generic extraction (fallback only) - also logs raw
- wait_for_user_input: For login/manual steps
- save_extraction_result: Save prompt/response pair with raw data
- get_current_state: Check task progress including raw data count
...
10. inject_prompt() ← Auto-adds unique marker!
11. observe_page("Verify prompt was injected with unique marker")
...
17. extract_ai_response() ← Logs all raw data!
18. save_extraction_result() if successful (includes raw data)

REMEMBER:
- observe_page is your eyes - use it constantly!
- The send button is an ARROW (→), not Enter key!
- inject_prompt adds unique markers automatically
- All extractions log raw data for analysis
...
class AgentExecutor:
    ...
    async def execute_iteration(self, deps: AgentDependencies) -> TaskStatus:
        ...
        - Raw data records: {len(deps.task_state.raw_extraction_data)}
        ...
        5. Remember: inject_prompt adds unique markers, extract_ai_response logs raw data
    ...
    async def execute_task(self, instructions: str, prompts: List[str]) -> None:
        ...
        try:
            for iteration in range(MAX_ITERATIONS):
                console.print(Panel(
                    f"🔄 Iteration {iteration + 1}/{MAX_ITERATIONS} | Queue: {len(deps.task_state.processing_queue)} items | Raw Data: {len(deps.task_state.raw_extraction_data)} records",
...
async def main() -> None:
    ...
    console.print(Panel(
        Text("🎭 Web Automation with Unique Markers & Raw Logging\n"
    ...
    if task_type == "test_weird":
        # Test prompts with weird content
        prompts = [
            "Write a haiku about a purple elephant eating quantum spaghetti",
            "Explain why rubber ducks are essential for debugging in exactly 42 words",
            "Create a recipe for invisible soup that tastes like nostalgia"
        ]
    ...
    4. Using inject_prompt() to insert the prompt with UNIQUE MARKER
    5. Clicking the send button (arrow → icon)
    6. Waiting for the response to generate
    7. Using extract_ai_response() to get the response with RAW DATA LOGGING
    8. Saving the result to JSONL (includes raw extraction data)
    9. Moving to next prompt

    CRITICAL: inject_prompt() adds unique markers automatically!
    CRITICAL: extract_ai_response() logs all raw data!
    ...
    summary_table.add_row("Special Features", "Unique Markers + Raw Data Logging")
    ...
    if Path(JSONL_OUTPUT_FILE).exists():
        ...
        results_table.add_column("Raw Data", style="magenta")
        ...
        for r in results[-5:]:  # Show last 5
            ...
            raw_data_count = len(r.get("raw_extraction_data", []))
            results_table.add_row(
                prompt_preview,
                f"{len(r['response'])} chars",
                f"{raw_data_count} records",
